<?php
/**
 * ViraCMS Administrator's Event Log Viewer Controller
 *
 ...
 */
class EventController extends VSystemLogController
{
  protected $accessRules = array(
    '*' => array('registryEventLog'),
  );
  ...
  protected function prepareDownloadFile()
  {
    ...
    foreach ($query as $row) {
      ...
      fputcsv($tmpFile, array(
        ...
        $row['source'],
        iconv(Yii::app()->charset, self::CSV_ENCODING, Yii::t($row['translate'], $row['event'], $params)),
  ...
  public function getTitle($view, $params = array())
  {
    ...
    switch ($view) {
      case 'index':
        $title[] = Yii::t('admin.registry.titles', 'Event Log');
        break;

      case 'clear':
        $title[] = Yii::t('admin.registry.titles', 'Clear Event Log');
        break;
    }
  ...
  {
    extract($params);
    return Yii::t('admin.registry.titles', 'Event log has been cleared');
  }

  public function getClearErrorMessage($params = array())
  {
    extract($params);
    return Yii::t('admin.registry.titles', 'An error occurred while clearing event log');
  }

  public function getModel($scenario = 'search')
  {
    return new VLogEvent($scenario);
  }

  public function getPlainModel()
  {
    return VLogEvent::model();
  }

  protected function afterClear($model)
  {
    parent::afterClear($model);
    Yii::app()->systemLog->logEvent('Event log has been cleared', array(), 'admin.registry.titles');
  }
}
