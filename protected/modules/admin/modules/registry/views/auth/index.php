    ...
    <div class="well">
      ...
      <form method="get" class="filter" id="form-filter">
        ...
        <div class="filter-text">
          <?= CHtml::activeTextField($model, 'authorID', array('class' => 'input-block-level')) ?>
        </div>
        <div class="nav-header"><?= Yii::t('admin.registry.filter', 'By Event Type') ?></div>
        <div class="filter-select">
          <?= CHtml::activeDropDownList($model, 'type', Yii::app()->collection->authLogType->toArray(), array('empty' => '', 'class' => 'input-block-level')) ?>
        </div>
        <div class="nav-header"><?= Yii::t('admin.registry.filter', 'By Event Success Result') ?></div>
        <div class="filter-select">
          <?= CHtml::activeDropDownList($model, 'result', Yii::app()->format->booleanFormat, array('empty' => '', 'class' => 'input-block-level')) ?>
        </div>
          ...
          <div class="row-fluid">
            <div class="span6">
              ...
              <label class="range">
                ...
                <input type="text" class="input-block-level" rel="<?= empty($model->time['start']) ? '' : $model->time['start'] ?>" data-datepicker="datepicker" data-format="<?= Yii::app()->format->dateFormat ?>" data-source="<?= CHtml::activeId($model, 'time[start]') ?>" value="<?= empty($model->time['start']) ? '' : Yii::app()->format->formatDate($model->time['start']) ?>" />
              </label>
            </div>
            <div class="span6">
              ...
              <label class="range">
                ...
                <input type="text" class="input-block-level" rel="<?= empty($model->time['end']) ? '' : $model->time['end'] ?>" data-datepicker="datepicker" data-format="<?= Yii::app()->format->dateFormat ?>" data-source="<?= CHtml::activeId($model, 'time[end]') ?>" value="<?= empty($model->time['end']) ? '' : Yii::app()->format->formatDate($model->time['end']) ?>" />
              </label>
            </div>
          </div>
        </div>
  ...
  <div class="span8">
    <div class="filtered-content">
      <div class="row-fluid log-header">
        ...
        <div class="span2"><h6><?= Yii::t('admin.registry.labels', 'Event Type') ?></h6></div>
        <div class="span1"><h6><?= Yii::t('admin.registry.labels', 'Success') ?></h6></div>
        <div class="span3"><h6><?= Yii::t('admin.registry.labels', 'Date and Time') ?></h6></div>
      </div>
      <?php
