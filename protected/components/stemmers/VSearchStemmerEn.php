 ...
 * Adopted from Jon Abernathy's stemmer @url http://www.chuggnutt.com/stemmer-source
 * Original CopyRights and Disclaimer follows
 *
 ...
 *  @version 2.1
 */
class VSearchStemmerEn implements VSearchStemmerInterface
{
  /**
   ...
   */
  protected function stem_list($words)
  {
    if (empty($words)) {
      return false;
    }
   ...
   */
  protected function _step_1($word)
  {
    // Step 1a
    if (substr($word, -1) == 's') {
   ...
   *  @return string Reduced word
   */
  protected function _step_2($word)
  {
   ...
   *  @return string Reduced word
   */
  protected function _step_3($word)
  {
   ...
   *  @return string Reduced word
   */
  protected function _step_4($word)
  {
    switch (substr($word, -2, 1)) {
      case 'a':
   ...
   *  @return string Reduced word
   */
  protected function _step_5($word)
  {
    if (substr($word, -1) == 'e') {
   ...
   *  @return boolean
   */
  protected function is_consonant($word, $pos)
  {
    // Sanity checking $pos
   ...
   *  @return integer
   */
  protected function count_vc($word)
  {
    $m = 0;
    $length = strlen($word);
    $prev_c = false;
   ...
   *  @return boolean
   */
  protected function _replace(&$word, $suffix, $replace, $m = 0)
  {
    $sl = strlen($suffix);
