...
abstract class TbInput extends CInputWidget
{
  ...
  public function init()
  {
    ...
    if (empty($this->htmlOptions['hint']) && method_exists($this->model, 'getAttributeHint')) {
      $this->htmlOptions['hint'] = $this->model->getAttributeHint($this->attribute);
    }

    if ($this->type === self::TYPE_UNEDITABLE) {
      if (isset($this->htmlOptions['class'])) {
        if (stripos($this->htmlOptions['class'], 'uneditable-') === false) {
