...
class Bootstrap extends CApplicationComponent
{
  ...
  /**
   * Registers the Bootstrap Datepicker Init JavaScript.
   * @param int $position the position of the JavaScript code.
   * @see CClientScript::registerScriptFile
   */
  public function registerDatepickerJS($position = CClientScript::POS_READY)
  {
    /** @var CClientScript $cs */
    $cs = Yii::app()->getClientScript();
    if (!$cs->isScriptRegistered('DatepickerInitScript')) {
      $cs->registerCoreScript('jquery');

      $config = array(
        'todayButtonText' => Yii::t('common', 'Today'),
        'clearButtonText' => Yii::t('common', 'Clear'),
        'monthNames' => array(
          Yii::t('common', 'January'),
      ...
      $cs->registerScript('DatepickerInitScript', "
$.extend($.fn.datepicker.defaults," . CJavaScript::encode($config) . ");
...
$('input[data-datepicker]').on('keydown paste',function(e)
{
	e.preventDefault();
	e.stopPropagation();
	return false;
});
$('input[data-datepicker]').on('change',function(e)
{
	var self = $(this);
	if (!self.val()) {
		self.attr('rel','');
	}
	$('#' + self.data('source')).val(self.attr('rel'));
});
	", $position);
    }
  }
